...
int Solution::canCompleteCircuit(const vector<int> &A, const vector<int> &B) {
    ...
    //traversing through the stations to find out the start
    for(int i=0;i<A.size();i++){
        left+=(A[i]-B[i]);
        //if the gas left <0 means it can't travel any longer
        //so we will start with this as the starting point and storing 
        //every left gas to check whether it can complete the circuit or not
        if(left<0){
            start=i+1;
            remain+=left;
            left=0;
    ...
    //it means that it can complete the whole circuit
    if(left+remain>=0){
        return start;
    }
    else
